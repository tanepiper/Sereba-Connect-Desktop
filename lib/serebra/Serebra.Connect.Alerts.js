...
Serebra.ForceUpdate=false;Serebra.ForceOffline=false;Serebra.JustLoaded=true;Serebra.LoggedIn=false;
Serebra.MessageCheckTime=300000;Serebra.NetworkOnline=false;Serebra.Password="";Serebra.PlayPopupSound=true;
Serebra.RememberMe=false;Serebra.UnreadMessages=false;Serebra.Username="";try{air.File.applicationStorageDirectory.resolvePath("update").deleteDirectory(true)
...
jQuery("alert",A).each(function(){var L=jQuery(this).attr("id");var H=jQuery("type",this).text();
var K=jQuery("alertText",this).text();var J=jQuery("userLink",this).text();var G=jQuery("objectLink",this).text();
var I=Serebra.Database.Query({queryString:"SELECT * FROM serebra_user_alerts WHERE AlertID = "+L});
if(!I.result.data){Serebra.Database.Query({queryString:"INSERT INTO serebra_user_alerts VALUES("+L+',"'+H+'","'+K+'","'+J+'","'+G+'",0)'});
C=C+1;Serebra.UnreadMessages=true}});if(Serebra.DisplayPop==="true"){var B="alerts";
var E=false;if(C==1){B="alert"}if(C==0&&Serebra.JustLoaded){C="no";E=true}else{if(C>0){E=true
}}if(E){Serebra.Chrome.Popup({message:'<h2>You have <span class="green">'+C+"</span> new "+B+"!</h2>",showLink:true,popupLife:6000})
}Serebra.JustLoaded=false}if(Serebra.UnreadMessages){function D(G){if(air.NativeApplication.supportsSystemTrayIcon){air.NativeApplication.nativeApplication.icon.bitmaps=new Array(G.target.content.bitmapData);
air.NativeApplication.nativeApplication.icon.tooltip="Serebra Connect Alerts - You have unread messages"
}}var F=new runtime.flash.display.Loader();F.contentLoaderInfo.addEventListener(air.Event.COMPLETE,D);
F.load(new air.URLRequest("app:/assets/images/icon_tray_newalerts.png"))}})};Serebra.Network.Monitor=null;
...
return false}function D(){B.target.window.nativeWindow.startMove()}function J(M){function L(){jQuery("tr#"+M,H).fadeOut(function(){jQuery(this).remove();
...
jQuery("#inner-table-wrapper",H).remove();var M=Serebra.Database.Query({queryString:"SELECT * FROM serebra_user_alerts ORDER BY AlertID DESC"});
...
N.load(new air.URLRequest("app:/assets/images/icon_desktop_16.png"))}jQuery("#outer-table-wrapper",H).append(K.join(""));
...
}})});return false});jQuery("#delete-all",H).unbind("click.deleteall").bind("click.deleteall",function(){Serebra.Chrome.ConfirmPrompt("Are you sure you wish to delete all messages?",function(){var O=Serebra.Database.Query({queryString:"SELECT * FROM serebra_user_alerts"});
...
var R=jQuery(this).attr("id");var P=Serebra.Database.Query({queryString:"SELECT * FROM serebra_user_alerts WHERE AlertID = "+R});
...
if(S=="true"){Serebra.Database.Query({queryString:"UPDATE serebra_user_alerts SET messageRead = 1 WHERE AlertID = "+R});
...
B.start()}};this.Initialise()};Serebra.Chrome.Settings=function(){this.windowLoaded=function(H){var F=H.target.window.nativeWindow;
...
}});if(!C){A.load(new air.URLRequest("app:/assets/html/Settings.html"))}};Serebra.Chrome.WhatsThis=function(A){this.Initialise=function(){var D=new air.NativeWindowInitOptions();
D.systemChrome=air.NativeWindowSystemChrome.NONE;D.type=air.NativeWindowType.LIGHTWEIGHT;
...
};this.CreateWindow=function(E){var H=jQuery("#message-popup",E.target.window.document).get(0);
function D(){E.target.window.close();return false}function C(){E.target.window.nativeWindow.startMove()
}function B(){jQuery("#window-handle",H).bind("mousedown.move",C);jQuery(".close-button",H).bind("click.close",D);
jQuery("#ok-button",H).bind("click.ok",D)}if(E.type==="complete"&&E.target.window.nativeWindow){var G=air.Screen.mainScreen.bounds.width/2;
var F=air.Screen.mainScreen.bounds.height/2;E.target.window.nativeWindow.x=G-(E.target.window.nativeWindow.width/2);
E.target.window.nativeWindow.y=F-(E.target.window.nativeWindow.height/2);B();E.target.window.nativeWindow.visible=true
}};this.Initialise()};