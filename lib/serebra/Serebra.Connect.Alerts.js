...
Serebra.DebugMode=false;Serebra.DisplayPopups=true;Serebra.DisplayPopupsAnswers=true;
Serebra.DisplayPopupsAwards=true;Serebra.DisplayPopupsBids=true;Serebra.DisplayPopupsMessages=true;
Serebra.DisplayPopupsQuestions=true;Serebra.Errors=[];Serebra.FirstRun=false;Serebra.ForceUpdate=false;
Serebra.ForceOffline=false;Serebra.IgnoreArray=[];Serebra.JustLoaded=true;Serebra.LoggedIn=false;
Serebra.MessageCheckTime=300000;Serebra.NetworkOnline=false;Serebra.Password="";Serebra.PlayPopupSound=true;
Serebra.RememberMe=false;Serebra.UnreadMessages=false;Serebra.Username="";Serebra.UserTable="serebra_user_";
...
})}})};Serebra.Network.CheckMessages=function(){Serebra.SOAP.GetUserAlerts({authCode:Serebra.AuthCode,applicationCode:Serebra.ApplicationCode},function(A){var C=0;
var E=0;jQuery("alert",A).each(function(){var M=jQuery(this).attr("id");var I=jQuery("type",this).text();
var L=jQuery("alertText",this).text();var K=jQuery("userLink",this).text();var H=jQuery("objectLink",this).text();
var J=Serebra.Database.Query({queryString:"SELECT * FROM "+Serebra.UserTable+" WHERE AlertID = "+M});
if(!J.result.data){Serebra.Database.Query({queryString:"INSERT INTO "+Serebra.UserTable+" VALUES("+M+',"'+I+'","'+L+'","'+K+'","'+H+'",0)'});
jQuery.each(Serebra.IgnoreArray,function(N,O){if(I===O[0]&&O[1]==="true"){C=C+1;Serebra.UnreadMessages=true
}else{E=E+1}})}});if(Serebra.DisplayPop==="true"){var B="alerts";var F=false;if(C==1){B="alert"
}if(C==0&&Serebra.JustLoaded&&E<5){C="no";F=true}else{if(C>0){F=true}}if(F){Serebra.Chrome.Popup({message:'<h2>You have <span class="green">'+C+"</span> new "+B+"!</h2>",showLink:true,popupLife:6000})
}Serebra.JustLoaded=false}if(Serebra.UnreadMessages){function D(H){if(air.NativeApplication.supportsSystemTrayIcon){air.NativeApplication.nativeApplication.icon.bitmaps=new Array(H.target.content.bitmapData);
air.NativeApplication.nativeApplication.icon.tooltip="Serebra Connect Alerts - You have unread messages"
}}var G=new runtime.flash.display.Loader();G.contentLoaderInfo.addEventListener(air.Event.COMPLETE,D);
G.load(new air.URLRequest("app:/assets/images/icon_tray_new.png"))}})};Serebra.Network.Monitor=null;
...
}});Serebra.UserTable="serebra_user_";Serebra.JustLoaded=true;Serebra.Network.MessageCheckTimer.stop();
Serebra.Menu.CreateLoginMenu()}}var B=new runtime.flash.display.Loader();B.contentLoaderInfo.addEventListener(air.Event.COMPLETE,A);
...
break;case"popupsound":Serebra.PlayPopupSound=D.value;break;case"show_answers":Serebra.DisplayPopupsAnswers=D.value;
break;case"show_awards":Serebra.DisplayPopupsAwards=D.value;break;case"show_bids":Serebra.DisplayPopupsBids=D.value;
break;case"show_messages":Serebra.DisplayPopupsMessages=D.value;break;case"show_questions":Serebra.DisplayPopupsQuestions=D.value;
...
return false}function D(){B.target.window.nativeWindow.startMove()}function J(M){function L(){jQuery("tr#"+M,H).remove();
I()}jQuery("tr#"+M,H).css({"background-color":"#f00"});var K=new air.Timer(1000,1);
K.addEventListener(air.TimerEvent.TIMER,L);K.start()}function I(){jQuery("#window-handle",H).unbind("mousedown.move").bind("mousedown.move",D);
...
if(P.result.data){var O=P.result.data[0].objectLink;Serebra.SOAP.ConsumeAlert({authCode:Serebra.AuthCode,applicationCode:Serebra.ApplicationCode,alertID:R},function(T){var S=jQuery("consumedAlert",T).text();
if(S=="true"){Serebra.Database.Query({queryString:"UPDATE "+Serebra.UserTable+" SET messageRead = 1 WHERE AlertID = "+R});
jQuery(".unread",Q).addClass("read").removeClass("unread");air.navigateToURL(new air.URLRequest(O))
}})}return false})}if(B.type==="complete"&&B.target.window.nativeWindow){var F=air.Screen.mainScreen.bounds.width/2;
...
}});if(!C){A.load(new air.URLRequest("app:/assets/html/LoginWindow.html"))}};Serebra.Chrome.Popup=function(A){this.Initialise=function(){var D=new air.NativeWindowInitOptions();
...
jQuery("#autologin",I).attr("checked",Serebra.AutoLogin);jQuery("#autostart",I).attr("checked",Serebra.AutoStart);
jQuery("#display-answers",I).attr("checked",Serebra.DisplayPopupsAnswers);jQuery("#display-awards",I).attr("checked",Serebra.DisplayPopupsAwards);
jQuery("#display-bids",I).attr("checked",Serebra.DisplayPopupsBids);jQuery("#display-messages",I).attr("checked",Serebra.DisplayPopupsMessages);
jQuery("#display-popup",I).attr("checked",Serebra.DisplayPopups);jQuery("#display-questions",I).attr("checked",Serebra.DisplayPopupsQuestions);
jQuery("#popup-sound",I).attr("checked",Serebra.PlayPopupSound);jQuery("#checktime option",I).each(function(){if(jQuery(this).val()==Serebra.MessageCheckTime){jQuery(this).attr("selected","selected")
}});jQuery(".save",I).bind("click.save",function(){Serebra.AutoLogin=jQuery("#autologin",I).attr("checked");
Serebra.AutoStart=jQuery("#autostart",I).attr("checked");Serebra.DisplayPopups=jQuery("#display-popup",I).attr("checked");
Serebra.DisplayPopupsAnswers=jQuery("#display-answers",I).attr("checked");Serebra.DisplayPopupsAwards=jQuery("#display-awards",I).attr("checked");
Serebra.DisplayPopupsBids=jQuery("#display-bids",I).attr("checked");Serebra.DisplayPopupsMessages=jQuery("#display-messages",I).attr("checked");
Serebra.DisplayPopupsQuestions=jQuery("#display-questions",I).attr("checked");Serebra.PlayPopupSound=jQuery("#popup-sound",I).attr("checked");
Serebra.RememberMe=jQuery("#rememberme",I).attr("checked");Serebra.MessageCheckTime=jQuery("#checktime",I).val();
Serebra.Database.SaveOrCreateOption({key:"autologin",value:Serebra.AutoLogin});Serebra.Database.SaveOrCreateOption({key:"autostart",value:Serebra.AutoStart});
...
Serebra.Network.MessageCheckTimer.start();F.target.window.close();return false});
F.target.window.nativeWindow.orderToFront()}if(F.type==="complete"&&F.target.window.nativeWindow){var H=air.Screen.mainScreen.bounds.width/2;
