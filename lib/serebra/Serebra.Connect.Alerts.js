...
break;default:break}return};Serebra.Menu.SystrayClickHandler=function(A){if(Serebra.NetworkOnline){Serebra.Chrome.AlertCenter()
...
}}});if(Serebra.UnreadMessages&&B>0){function C(E){if(air.NativeApplication.supportsSystemTrayIcon){air.NativeApplication.nativeApplication.icon.bitmaps=new Array(E.target.content.bitmapData);
air.NativeApplication.nativeApplication.icon.tooltip="Serebra Connect Alerts - You have unread messages"
}if(Serebra.DisplayPop==="true"){Serebra.Chrome.Popup({message:'<h2>You have <span class="green">'+B+"</span> new alerts!</h2>",showLink:true,popupLife:6000})
}}var D=new runtime.flash.display.Loader();D.contentLoaderInfo.addEventListener(air.Event.COMPLETE,C);
D.load(new air.URLRequest("app:/assets/images/icon_tray_newalerts.png"))}})};Serebra.Network.Monitor=null;
...
}});Serebra.Chrome.MessagePopup({message:"<h2>Downloading Update</h2>",showLink:false,popupLife:6000});
...
G.target.window.nativeWindow.y=0;G.target.window.nativeWindow.addEventListener(air.Event.ACTIVATE,D);
